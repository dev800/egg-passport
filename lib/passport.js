...
class EggPassport extends Passport {
  ...
  mount(strategy, options = {}) {
    ...
    this.app.get(url.parse(options.loginPath || `/passport/${strategy}`).pathname, auth);
    this.app.get(url.parse(options.callbackPath || `/passport/${strategy}/callback`).pathname, auth);
  }
